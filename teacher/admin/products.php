...
// for sizes and quantity preview
  ...
  $list_price = ((isset($_POST['unit_amount']) && $_POST['unit_amount'] != '')?sanitize($_POST['unit_amount']):'');
  ...
  if(isset($_GET['edit']))
  {
    ...
    $brand =((isset($_POST['brand']) && $_POST['brand'] != '')?sanitize($_POST['brand']):$product_res['brand']);
    ...
    $list_price = ((isset($_POST['unit_amount']) && $_POST['unit_amount'] != '')?sanitize($_POST['unit_amount']):$product_res['list_price']);
    $description = ((isset($_POST['description']) && $_POST['description'] != '')?sanitize($_POST['description']):$product_res['description']);
...
if($_POST){
  $errors = array();
  $required = array('title','brand','price','parent','child','qamount'); // here qamount is sizes
  ...
  if (!empty($_FILES)) {
    ...
    $upload_path = BASEURL.'image/products/'.$upload_name;
    $dbpath = '/teacher/image/products/'.$upload_name;
  ...
  if(!empty($errors)){
    ...
    $insertSql = "INSERT INTO products (`title`,`price`,`list_price`,`brand`,`categories`,`sizes`,`image`,`description`)
     VALUES ('$title','$price','$list_price','$brand','$category_var','$sizes_qamount','$dbpath','$description')";
     if(isset($_GET['edit']))
     {
       $insertSql = "UPDATE products SET title = '$title', price = '$price', list_price = '$list_price',
       brand = '$brand', categories = '$category_var', sizes = '$sizes_qamount', image = '$dbpath', description = '$description'
       WHERE id = '$edit_id'";
...
<h2 class="text-center"><?=((isset($_GET['edit']))?'Edit':'Add A New'); ?> Product </h2><hr>
  <form action="products.php?<?=((isset($_GET['edit']))?'edit='.$edit_id:'add=1'); ?>" method="POST" enctype="multipart/form-data">
    ...
    <div class="form-group col-md-3">
      <label for="qamount">Sizes & Quantity preview:</label>
